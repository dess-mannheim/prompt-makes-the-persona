"""
Cheng et al. Marked Personas method to obtain the words that distinguish a target group from the corresponding unmarked ones
"""
...
def marked_words(df, target_val, target_col, unmarked_val,verbose=False):
    ...
    for i in range(len(unmarked_val)):
        # self description tasl
        delt = get_log_odds(subdf['self_description'], df.loc[df[target_col[i]]==unmarked_val[i]]['self_description'],df['self_description'],verbose) #first one is the positive-valued one
        # social media bio task
        #delt = get_log_odds(subdf['bio'], df.loc[df[target_col[i]]==unmarked_val[i]]['bio'],df['bio'],verbose) #first one is the positive-valued one
        c1 = []
        c2 = []
